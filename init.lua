...
local function enable(self,config,stage,install)
  ...
  install(function()
    ...
    observe(sites.beginMatch.address,5,function() loadLifecycle:cancel(); recorder:beginMatch() end)
    observe(sites.prepareMatch.address,10,function()
      if engine.trace then engine.trace:observe('stop','new match') end
      recorder:prepareRecording()
      ui.resetButtons()
    end)
    observe(sites.menuTransition.address,6,function(registers)
      matchResults:onMenuView(registers.EBP)
      recorder:onMenuView(registers.EBP)
    end)
    observe(sites.loadBegin.address,6,function()
      loadLifecycle:begin()
    end)
    ...
    observe(sites.resetMatch.address,5,function() recorder:reset() end)
