...
Optional **Multiplayer diagnostics** logs actual command execution for comparing two peers. It is disabled by default and does not enable multiplayer replay. See [multiplayer trace instructions](docs/multiplayer-diagnostics.md).
...
The builder creates `dist/recorder-0.9.0.zip` with a flat module layout. `definition.yml` uses metadata schema version `1.0.0`; that is separate from the extension version.
