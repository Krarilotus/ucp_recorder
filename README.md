...
Version 0.14.0 isolates eight more audio/UI RNG calls that playback does not reproduce, including resource-warning speech, wall-clock-driven music and heads-on-spikes selection. It includes 0.13.0's multiplayer roster and immediate-command diagnostics. Recordings, the native replay browser and loading a starting save have been exercised in game. **A complete replay with the latest RNG fixes, Extreme gameplay and settings restart still need live verification.** Multiplayer recording is not supported yet; normal multiplayer isolation has automated coverage but still needs a live comparison. See the [presentation RNG audit](docs/presentation-rng.md) and [live findings and next tests](docs/live-validation.md).
...
The builder creates `dist/recorder-0.14.0.zip` with a flat module layout. `definition.yml` uses metadata schema version `1.0.0`; that is separate from the extension version.
