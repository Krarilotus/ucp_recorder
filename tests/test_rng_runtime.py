    ...
    def test_extension_only_require_and_checksum_agree_across_runtimes(self):
        ...
        for runtime in (Lua54,LuaJIT):
            with self.subTest(runtime=runtime):
                ...
                lua.globals().fireSource=(Path(__file__).resolve().parents[1]/'code/rng-fire-context.lua').read_text()
                lua.execute('''
require=function(name)
 if name=='code/rng-spawn-context' then return assert((loadstring or load)(spawnSource))() end
 if name=='code/rng-fire-context' then return assert((loadstring or load)(fireSource))() end
