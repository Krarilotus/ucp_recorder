"""Use shipped UCP cached discovery without an extra scan or newer API."""
...
local calls=0
core={AOBScanUnique=function() error('unreleased API used') end,
 scanForAOB=function() error('uncached second scan used') end,
...
for _,failure in ipairs({'missing','ambiguous','inaccessible'}) do
 ...
 assert(not ok and tostring(reason):find('fixture: native context not found',1,true))
end
