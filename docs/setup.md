...
2. Choose the release for that PR's current commit: tags look like
   `pr-<number>-<commit>`. Under **Assets**, download **`recorder-0.26.0.zip`**
...
4. For manual installation, put the ZIP at
   `<test game>/ucp/modules/recorder-0.26.0.zip`. Keep it zipped. This is a
   **module**, not a plugin; `definition.yml` and `init.lua` are at the ZIP root.
5. Reload the extension list, enable **UCP-Recorder**, and select **0.26.0**.
...
8. Launch through UCP. Check the console/log says
   `enabling extension: recorder version: 0.26.0`. Since 0.26.0,
...
the definition file format; `version: 0.26.0` is the extension version, and
