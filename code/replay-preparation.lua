...
function M.prepare(id,engine,worlds,progress)
  ...
  local snapshotPath,snapshotHash,rng=path..'/start.sav',manifest.snapshotHash
  ...
  if not rng then
    rng=require('code/world-reader').read(path..'/rng.bin',0x9c50)
...
-- The full recovery chain was admitted before entering the first world. Reuse
-- those validated streams, environment and RNG blocks instead of rescanning
-- every recorded command and installed asset on each recovery transition.
function M.segment(first,id)
  local world=assert(first.worlds and first.worlds[id],'Recovery world was not prepared')
  assert(world.manifest and world.manifest.id==id and world.rng,'Recovery segment was not validated')
  return {manifest=world.manifest,snapshotPath=world.path,snapshotHash=world.hash,
    rng=world.rng,worlds=first.worlds,info=first.info}
end
