...
function M.new(action,clock)
  ...
  self.thread=coroutine.create(function()
    return action(function(message)
      ...
      -- Progress frequency is not a work budget: short rows must not each cost
      -- a menu frame. Yield only when this slice has actually used its time.
      if (self.clock()-self.sliceStarted)%4294967296>=M.BUDGET_MS then
        coroutine.yield()
        assert(not self.cancelled,'Replay preparation cancelled')
      end
    end)
